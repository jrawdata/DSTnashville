...
traffic$date_and_time_formatted <-factor(traffic$date_and_time_formatted, levels = unique(traffic$date_and_time_formatted))
...
feb_calls <- calls %>% 
  filter(month(call_rec) == 2)

march_calls <- calls %>% 
  filter(month(call_rec) == 3)

april_calls <- calls %>% 
  filter(month(call_rec) == 4)

may_calls <- calls %>% 
 filter(month(call_rec) == 5) 

june_calls <- calls %>% 
 filter(month(call_rec) == 6)

july_calls <- calls %>% 
 filter(month(call_rec) == 7)

aug_calls <- calls %>% 
  filter(month(call_rec) == 8)

sept_calls <- calls %>% 
  filter(month(call_rec) == 9)

oct_calls <- calls %>% 
  filter(month(call_rec) == 10)

nov_calls <- calls %>% 
  filter(month(call_rec) == 11)

dec_calls <- calls %>% 
  filter(month(call_rec) == 12)
```
```{r}
...
calls$call_rec_formatted <-factor(calls$call_rec_formatted, levels = unique(calls$call_rec_formatted))
